...
#define _CLAMP(a, minimum, maximum) std::min(std::max(a, minimum), maximum)

__forceinline DWORD GetSleepTime(DWORD Frames, DWORD SampleRate)
{
    float fRet = 0.f;
    if (!SampleRate)
        return 0;
...
__forceinline void BaseToStream(f32 In, void* pOut, int Type)
{
    switch (Type)
    {
    /*
    ...
    */
    case 1:
    {
        short* pNum = (short*)pOut;
        *pNum = _CLAMP(((short)(In * 32768.0f)), (short)-32768, (short)32767);
    }
    /*
        24 bit int type
    */
...
__forceinline f32 BaseToFloat(void* pIn, int Type)
{
    switch (Type)
    {
    /*
    ...
    */
    case 1:
    {
        short* pNum = (short*)pIn;
        return (float)*pNum / 32768.0f;
    }
    /*
        24 bit int type
    */
...
__forceinline void ConvertToStream(f32* In, void* Out, int Frames, int Type)
{
    switch (Type)
    {
    case 0:
        /*
            Single channels frames (must mul by channels if stereo or surround)
        */
        memcpy(In, Out, sizeof(f32) * Frames);
        break;
    default:
    {
        short* pShort = (short*)Out;

        for (size_t i = 0; i < Frames; i++)
        {
            BaseToStream(In[i], &pShort[i], Type);
        }
    }
    break;
    }
